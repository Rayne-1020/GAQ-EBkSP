    ...
    def run(self, num_runs, rl_actions=None, convert_to_csv=False):
        ...
        for i in range(num_runs):
            ...
            for j in range(num_steps):
                ...
                state, reward, done, infos = self.env.step(rl_actions(state))
                #print(reward,infos)
                #state = self.env.step(rl_actions(state))[0]
                #reward = self.env.step(rl_actions(state))[1]
                #print(state)
                ...
                # if j % 200 == 0 and j == 0:
                #     state = self.env.initial_state#get the observation from reset() without run the reset()
                #     Reward = 0
                #     done = False
                #     infos = {}
                ...
                #     done = done = (len(self.env.departed_vehicles) == len(self.env.arrived_vehicles) and len(self.env.arrived_vehicles)>200)#number of vehicles added into the network
                ...
                ret += Reward
                number_steps += 1
                # self.env.rule_based_reward.append(ret)
                ...
                if done:
                    # print(self.env.rule_based_reward)
                    break
            ...
            self.env.rule_based_reward.append(ret)
            self.env.rule_based_steps.append(number_steps)
            print(self.env.rule_based_reward)
            print(self.env.rule_based_steps)
