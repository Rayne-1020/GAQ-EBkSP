...
#from flow.envs import routes_generation
from routes_generation import *

from flow.networks import Network
from flow.core.params import InitialConfig,VehicleParams,NetParams
...
class Env(gym.Env, metaclass=ABCMeta):
    ...
    * compute_reward
    ...
    def __init__(self,
        ...
        #self.rl_actions_laststep = None
        self.average_distance_to_destination = None

        self.arrived_vehicles = []
        self.departed_vehicles = []


        #store the fog area weight
        # self.Fog_A_weight = None
        # self.Fog_B_weight = None
        # self.Fog_C_weight = None
        # self.Fog_D_weight = None
        # self.Fog_E_weight = None
        # self.Fog_F_weight = None
        ...
        self.rule_based_steps = []
        # self.model_reward = []

        self.destination_edge = None

        #initial_state get from reset() observation
        self.initial_state = None

        self.average_speed = None

        self.average_V_C = None

        #self.back_available_routes = None
    ...
    def step(self, rl_actions):
        ...
        #     done = False
        #     infos = {}
        #     return states, reward, down, infos

        # if self.time_counter % 200 == 0 and self.time_counter != 0:
        ...
        #     states = self.reset()
        # print(states)
        #updated_routes = self.get_updated_route()
        ...
        Destination_Difference = self.vehicle_current_edge()
        # if self.average_speed == 0:#sometimes the update routes is stucked and no vehicle moves, then stop the episode
        #     done = True
        # else:
        #     done = False
        if (len(self.departed_vehicles) == len(self.arrived_vehicles) and len(self.arrived_vehicles)>200):
            done = True
        else:
            done = False
        if (Destination_Difference == 0) or self.average_speed == 3.0:
            done = True
        else:
            done = False
        ...
        # done = (len(self.departed_vehicles) == len(self.arrived_vehicles) and len(self.arrived_vehicles)>200)#number of vehicles added into the network
        self.done = done
        ...
        Reward = self.compute_reward_no_rl_actions()
        # if self.done == True:
        #     print(Reward)
        #test
    ...
    def reset(self):
        ...
        self.departed_vehicles = []
        #left bottom nodes
        Left_bottom_Nodes = ['C144','C143']
        #background vehicle destination distinguish different area?
        Back_target_right_to_left = ['C162']
        Back_target_left_to_right = ['C23']
        Back_target_middle_to_up = ['C104']
        #Back_target_middle_to_down = ['C146']

        target = []
        target.append(random.choice(Left_bottom_Nodes))
        ...
        # target = ['C144']
        print(target)
        Back_target_left = target


        #regenerate the Pair dictionary
        Pair = {}
        count = 0
        ...
        self.Pair = Pair
        #regenerate the new available paths with edges
        Sumo_Path = []
        node_path = []
        ...
        for i in range(0,len(EDGES_DISTRIBUTION)):
            for j in range(0,len(Sumo_Path)):
                if Sumo_Path[j][0] == EDGES_DISTRIBUTION[i]:
                    ...
                    Edges_pop = (Sumo_Path[j],Probability_EDGES_DISTRIBUTION[i])
                    EDGES.append(Edges_pop)
        Last_Edges = []

        for i in range(0,len(Sumo_Path)):
            Last_Edges.append(Sumo_Path[i][-1])
        Destination = unique(Last_Edges)
        self.destination_edge = Destination
        #print(Destination)
        ...
        for i in range(len(EDGES_DISTRIBUTION)):
            ...
            new_rts[EDGES_DISTRIBUTION[i]] = group_EDGES

        #regenerate the Pair dictionary
        Back_Pair_right_to_left = {}
        back_count = 0
        for j in range(0,len(list(G_network.edges))):
            if nx.has_path(G_network,list(G_network.edges)[j][1],Back_target_right_to_left[0]) == True:
                back_count += 1
                #print(list(G_network.edges)[j][1],target[i])
                Back_Pair_right_to_left[count] = (list(G_network.edges)[j][0],list(G_network.edges)[j][1],Back_target_right_to_left[0])
        self.Back_Pair_right_to_left = Back_Pair_right_to_left
        #regenerate the new available paths with edges
        Back_Sumo_Path = []
        Back_node_path = []

        for i in range (0,len(Back_target_right_to_left)):#target 
            for j in range(0,len(list(G_network.edges))):
                if nx.has_path(G_network,list(G_network.edges)[j][1],Back_target_right_to_left[i]) == True: #and list(G_network.get_edge_data(list(G_network.edges)[j][0],list(G_network.edges)[j][1]).values())[1] == '46522160#0':
                    #print('true')
                    for path in k_shortest_paths(G_network, list(G_network.edges)[j][1], Back_target_right_to_left[i], 3, weight = 'weight'):
        ...
        #                     Weights.append(weights)
                        ...
                        Back_Sumo_Path.append(Path)
        #updated Edge_distribution and Edges
        FIRST_EDGES = []
        for i in range(0,len(Back_Sumo_Path)):
            FIRST_EDGES.append(Back_Sumo_Path[i][0])
        EDGES_DISTRIBUTION = unique(FIRST_EDGES)
        #update the probabilioty: choose the least popular routes
        Probability_EDGES_DISTRIBUTION = []
        for i in range(0,len(EDGES_DISTRIBUTION)):
            back_count = 0
            for j in range(0,len(Back_Sumo_Path)):
                if Back_Sumo_Path[j][0] == EDGES_DISTRIBUTION[i]:
                    back_count += 1
            probability = 1/count
            Probability_EDGES_DISTRIBUTION.append(probability)
        ...
        for i in range(0,len(EDGES_DISTRIBUTION)):
            for j in range(0,len(Back_Sumo_Path)):
                if Back_Sumo_Path[j][0] == EDGES_DISTRIBUTION[i]:
                    #print(EDGES_DISTRIBUTION[i])
                    Edges_pop = (Back_Sumo_Path[j],Probability_EDGES_DISTRIBUTION[i])
                    EDGES.append(Edges_pop)
        Last_Edges = []

        for i in range(0,len(Back_Sumo_Path)):
            Last_Edges.append(Back_Sumo_Path[i][-1])
        Destination = unique(Last_Edges)
        self.destination_edge = Destination
        #print(Destination)
        
        #new rts right
        back_new_rts_right_to_left = {}
        for i in range(len(EDGES_DISTRIBUTION)):
            group_EDGES = []
            for j in range(len(EDGES)):
                if EDGES[j][0][0] == EDGES_DISTRIBUTION[i]:
                    group_EDGES.append(EDGES[j])
            back_new_rts_right_to_left[EDGES_DISTRIBUTION[i]] = group_EDGES


        #regenerate the Pair dictionary
        Back_Pair_left_to_right = {}
        back_count = 0
        for j in range(0,len(list(G_network.edges))):
            if nx.has_path(G_network,list(G_network.edges)[j][1],Back_target_left_to_right[0]) == True:
                back_count += 1
                #print(list(G_network.edges)[j][1],target[i])
                Back_Pair_left_to_right[count] = (list(G_network.edges)[j][0],list(G_network.edges)[j][1],Back_target_left_to_right[0])
        self.Back_Pair_left_to_right = Back_Pair_left_to_right
        #regenerate the new available paths with edges
        Back_Sumo_Path = []
        Back_node_path = []

        for i in range (0,len(Back_target_left_to_right)):#target 
            for j in range(0,len(list(G_network.edges))):
                if nx.has_path(G_network,list(G_network.edges)[j][1],Back_target_left_to_right[i]) == True: #and list(G_network.get_edge_data(list(G_network.edges)[j][0],list(G_network.edges)[j][1]).values())[1] == '46522160#0':
                    #print('true')
                    for path in k_shortest_paths(G_network, list(G_network.edges)[j][1], Back_target_left_to_right[i], 3, weight = 'weight'):
        ...
        #new rts from left to right
        back_new_rts_left_to_right = {}
        for i in range(len(EDGES_DISTRIBUTION)):
            group_EDGES = []
            for j in range(len(EDGES)):
                if EDGES[j][0][0] == EDGES_DISTRIBUTION[i]:
                    group_EDGES.append(EDGES[j])
            back_new_rts_left_to_right[EDGES_DISTRIBUTION[i]] = group_EDGES

        #middle
        #regenerate the Pair dictionary
        Back_Pair_middle_to_up = {}
        back_count = 0
        for j in range(0,len(list(G_network.edges))):
            if nx.has_path(G_network,list(G_network.edges)[j][1],Back_target_middle_to_up[0]) == True:
                back_count += 1
                #print(list(G_network.edges)[j][1],target[i])
                Back_Pair_middle_to_up[count] = (list(G_network.edges)[j][0],list(G_network.edges)[j][1],Back_target_middle_to_up[0])
        self.Back_Pair_middle_to_up = Back_Pair_middle_to_up
        #regenerate the new available paths with edges
        Back_Sumo_Path = []
        Back_node_path = []

        for i in range (0,len(Back_target_middle_to_up)):#target 
            for j in range(0,len(list(G_network.edges))):
                if nx.has_path(G_network,list(G_network.edges)[j][1],Back_target_middle_to_up[i]) == True: #and list(G_network.get_edge_data(list(G_network.edges)[j][0],list(G_network.edges)[j][1]).values())[1] == '46522160#0':
                    #print('true')
                    for path in k_shortest_paths(G_network, list(G_network.edges)[j][1], Back_target_middle_to_up[i], 3, weight = 'weight'):
        ...
        #new rts right
        back_new_rts_middle_to_up = {}
        for i in range(len(EDGES_DISTRIBUTION)):
            group_EDGES = []
            for j in range(len(EDGES)):
                if EDGES[j][0][0] == EDGES_DISTRIBUTION[i]:
                    group_EDGES.append(EDGES[j])
            back_new_rts_middle_to_up[EDGES_DISTRIBUTION[i]] = group_EDGES

        #updated the available_routes as new_rts
        #updated the background vehicles' available routes
        self.available_routes = new_rts
        self.back_available_routes_right = back_new_rts_right_to_left
        self.back_available_routes_left = back_new_rts_left_to_right
        self.back_available_routes_middle_up = back_new_rts_middle_to_up
