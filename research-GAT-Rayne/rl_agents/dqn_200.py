...
class AbstractDQNAgent(Agent):
    """Write me
    """
    def __init__(self, nb_actions, memory, gamma=.99, batch_size=32, nb_steps_warmup=2000,
                 train_interval=1, memory_interval=1, target_model_update=1000,
    ...
    def compute_batch_q_values(self, state_batch):
        ...
        assert q_values.shape == (len(state_batch), 6, self.nb_actions)
        return q_values

    def compute_q_values(self, state):
        q_values = self.compute_batch_q_values([state])[0]
        assert q_values.shape == (6, self.nb_actions)
        return q_values
...
class DQNAgent(AbstractDQNAgent):
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        y_true = Input(name='y_true', shape=(6, self.nb_actions))
        mask = Input(name='mask', shape=(6, self.nb_actions))
    ...
    def forward(self, observation):
        ...
        #state = state[0]
        print(self.training, self.step)

        if self.training:
            if self.step < self.nb_steps_warmup and self.start_policy:#self.step%self.train_interval == 0 and 
    ...
    def backward(self, reward, terminal):
        #print("backward_reward",reward)
        # Store most recent experience in memory.
        if self.step % self.memory_interval == 0:
            #print("recent_observation",self.recent_observation)
            #assert len(self.recent_observation) == 2
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            #print("terminal1_batch",terminal1_batch, terminal1_batch.shape)
            ...
            if self.enable_double_dqn:
                ...
                assert q_values.shape == (self.batch_size, 6, self.nb_actions)
                #print("q_values",q_values)
                actions = np.argmax(q_values, axis=-1)  #(batch, )
                #print("actions_axis-1",actions)
                assert actions.shape == (self.batch_size, 6)
                

                # Now, estimate Q values using the target network but select the values with the
                # highest Q value wrt to the online model (as computed above).
                target_q_values = self.target_model.predict_on_batch(state1_batch)
                assert target_q_values.shape == (self.batch_size, 6, self.nb_actions)
            ...
            assert q_batch.shape == (self.batch_size,6)
            #print(q_batch, q_batch.shape)

            targets = np.zeros((self.batch_size, 6, self.nb_actions))
            dummy_targets = np.zeros((self.batch_size, 6))
            masks = np.zeros((self.batch_size, 6, self.nb_actions))

            # Compute r_t + gamma * max_a Q(s_t+1, a) and update the target targets accordingly,
            # but only for the affected output units (as given by action_batch).

            discounted_reward_batch = self.gamma * q_batch  # (batch_size * nb_agents)
            #print("discounted_reward_batch",discounted_reward_batch)

            # Set discounted reward to zero for all states that were terminal.
            discounted_reward_batch *= (terminal1_batch.reshape(-1,1).dot(np.ones([1,6]))) # (batch_size * nb_agents
            #print(discounted_reward_batch)
            #assert discounted_reward_batch.shape == reward_batch.shape
            Rs = reward_batch.reshape(-1,1) + discounted_reward_batch # (batch_size * nb_agemts)
            assert Rs.shape == (self.batch_size, 6)
            ...
            for idx, (target, mask, R, action) in enumerate(zip(targets, masks, Rs, action_batch)):
                ...
                #print(num, num.shape)
                target[num,action] = R[action].reshape(1,-1)
                dummy_targets[num] = R.reshape(1,-1)
                mask[num,action] = 1.
                #print("target",target)
                # for num in range(0,5):
                #     target[num, action] = R[action].reshape(1,-1)
                #     #print(target,target[action],target[num, action],R[action].shape)
                #     dummy_targets[num] = R.reshape(1,-1)
                #     mask[num, action] = 1.
                
                # target[action] = R[action].dot(np.ones([1,5])) # update action with estimated accumulated reward
                # dummy_targets[idx] = R.reshape(1,-1)
                # mask[idx, action] = 1.  # enable loss for this specific action
            # for idx, (target, mask, R, action) in enumerate(zip(targets, masks, Rs, action_batch)):
            #     print(target, R)
...
class NAFAgent(AbstractDQNAgent):
    ...
    def forward(self, observation):
        ...
        # Book-keeping.
        self.recent_observation = observation
        self.recent_action = self.select_action(state)
